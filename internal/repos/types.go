...
func GrantedScopes(logger log.Logger, ctx context.Context, cache ScopeCache, db database.DB, svc *types.ExternalService) ([]string, error) {
	externalServicesStore := db.ExternalServices()
	if svc.IsSiteOwned() || (svc.Kind != extsvc.KindGitHub && svc.Kind != extsvc.KindGitLab) {
		return nil, nil
	}
	src, err := NewSource(logger, ctx, db, svc, nil)
	if err != nil {
	...
	case *GitLabSource:
		...
		// Slow path
		src, err := NewGitLabSource(logger, ctx, db, svc, nil)
		if err != nil {
			return nil, errors.Wrap(err, "creating source")
		}
