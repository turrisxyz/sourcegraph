...
func parseYarnLockFile(r io.Reader) (deps []reposource.PackageDependency, graph *DependencyGraph, err error) {
	...
	*/
	...
	var (
		...
		dependencyNames = map[*reposource.NpmDependency][]*reposource.NpmDependency{}
	)
	...
	for scanner.Scan() {
		...
		if version, err = getVersion(line); err == nil { // e.g. version: 2.0.6
			...
			if err != nil {
				...
				byName[name] = dep
				current = dep
				dependencyNames[current] = []*reposource.NpmDependency{}
				name = ""
			}
			continue
		}
		...
		if line[:4] == "    " && parsingDependencies && current != nil {
			...
			dep, err := reposource.ParseNpmDependency(depName + "@" + depVersion)
			if err != nil {
				errs = errors.Append(errs, err)
			}

			if deps, ok := dependencyNames[current]; !ok {
				dependencyNames[current] = []*reposource.NpmDependency{dep}
			} else {
